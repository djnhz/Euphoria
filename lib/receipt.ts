...
export async function pdfTekst(url: string): Promise<PdfResultaat> {
  ...
  try {
    // pdfjs zoekt zijn worker zelf naast dit bestand op. Dat pad klopt; het bestand
    // moest alleen meegedeployed worden, zie outputFileTracingIncludes in
    // next.config.ts. Zelf een pad zetten werkt hier juist averechts: de bundler
    // vervangt `require.resolve` door een intern modulenummer.
